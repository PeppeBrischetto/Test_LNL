...
: G4VModularPhysicsList(){
  ...
  // Look also TRDetectorConstruction class to set the user limits and
  // attach them to a certain volume (G4UserLimits)
...
void TRPhysicsList::SetCuts()
{
  ...
  /*
  //defaultCutValue = 1.*km; //1*mm
  //SetCutsWithDefault();
  SetCutValue(1.*km, "gamma");
  SetCutValue(1.*km, "e-");

  G4Region* towersRegion  = G4RegionStore::GetInstance()->GetRegion("towers_region");
  G4ProductionCuts* cuts = new G4ProductionCuts;
  cuts->SetProductionCut(1.*mm);
  towersRegion->SetProductionCuts(cuts);
  */
